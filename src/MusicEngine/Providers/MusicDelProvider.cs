...
/// <summary>
/// MusicDel (musicdel.ir) — download tier, fully domestic.
/// Search parses the site's post links; each track page contains download buttons
/// for 320, 128, and 64 kbps. The CDN uses Solar Hijri (Jalali) calendar dates.
/// CDN structure: dl.musicdel.ir/Music/{year}/{month}/{day}/{Artist} - {Title} (320).mp3
/// </summary>
public sealed partial class MusicDelProvider : ISearchProvider, IDownloadProvider
{
    private const string Host = "https://musicdel.ir";
    private const string CdnHost = "https://dl.musicdel.ir";
    ...
    private readonly ILogger<MusicDelProvider> _logger;

    public ProviderId Id => ProviderId.MusicDel;
    public string DisplayName => "MusicDel";
    public SearchTier Tier => SearchTier.DownloadOnly;
    public bool IsAvailable => true;

    public MusicDelProvider(SharedHttpClient http, ILogger<MusicDelProvider>? logger = null)
    {
        // insecureTls: CDN pages may serve self-signed certs (BUG-13 family).
        _http = http.Create("MusicDel", insecureTls: true);
        SharedHttpClient.ApplyBrowserHeaders(_http, "https://musicdel.ir/");
        _logger = logger ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<MusicDelProvider>.Instance;
    }

    public bool CanDownload(SearchResult result) => result.Provider == ProviderId.MusicDel;
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        if (html.Length == 0)
        {
            _logger.LogDebug("MusicDel empty response for {Query}", query);
            yield break;
        }

        var doc = new HtmlDocument();
        doc.LoadHtml(html);
        // musicdel.ir result entries: <a href="https://musicdel.ir/single-tracks/{id}/">
        var postNodes = doc.DocumentNode.SelectNodes("//a[contains(@href,'/single-tracks/')]");
        ...
        foreach (var node in postNodes)
        {
            ...
            if (!uri.Host.Contains("musicdel.ir")) continue;
            ...
            if (lastSegment.Length < 3) continue;

            var track = await GetTrackAsync(uri.AbsoluteUri, lastSegment, ct).ConfigureAwait(false);
            if (track is null) continue;

            yield return new SearchResult
            {
                Provider = ProviderId.MusicDel,
                Id = uri.AbsolutePath,
    ...
    {
        progress?.Report(new DownloadProgress(DownloadPhase.Resolving, 0, null, "MusicDel: resolving URL"));
        ...
        {
            _logger.LogDebug("MusicDel re-resolve failed ({Msg}); falling back to stored URL", ex.Message);
        }
        finalUrl ??= track.DirectStreamUri?.OriginalString;
        if (string.IsNullOrEmpty(finalUrl))
            throw new InvalidOperationException("No download URL on MusicDel page.");
        ...
        return new DownloadResult(finalPath, StreamQuality.Maximum256K, ProviderId.MusicDel);
    }
    ...
    private async Task<string> GetStringAsync(string url, CancellationToken ct)
    {
        ...
        {
            _logger.LogDebug("MusicDel HTTP failure for {Url}: {Msg}", url, ex.Message);
            return "";
        }
    }
    ...
    private async Task<TrackLink?> GetTrackAsync(string trackPageUrl, string slugText, CancellationToken ct)
    {
        ...
        // Look for download links in the page - MusicDel uses JavaScript to construct URLs
        var links = doc.DocumentNode.SelectNodes("//a[contains(@href,'dl.musicdel.ir')]");
        if (links is null)
        {
        ...
        foreach (var link in links)
        {
            ...
            if (href.Contains("dl.musicdel.ir") && href.EndsWith(".mp3", StringComparison.OrdinalIgnoreCase))
            {
                bestUrl = href;
    ...
    [GeneratedRegex("href=\"(https?://dl\\.musicdel\\.ir[^\"]+\\.mp3)\"")]
