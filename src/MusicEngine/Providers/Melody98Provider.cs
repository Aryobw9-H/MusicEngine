...
/// <summary>
/// Melody98 (melody98.ir) — download tier, fully domestic.
/// Search parses the site's post links; each track page contains download buttons
/// for 320 and 128 kbps. The CDN uses Solar Hijri (Jalali) calendar dates.
/// CDN structure: dl.melody98.ir/music/{year}/{month}/{day}/{Artist} - {Title} [320].mp3
/// </summary>
public sealed partial class Melody98Provider : ISearchProvider, IDownloadProvider
{
    private const string Host = "https://melody98.ir";
    private const string CdnHost = "https://dl.melody98.ir";
    ...
    private readonly ILogger<Melody98Provider> _logger;

    public ProviderId Id => ProviderId.Melody98;
    public string DisplayName => "Melody98";
    public SearchTier Tier => SearchTier.DownloadOnly;
    public bool IsAvailable => true;

    public Melody98Provider(SharedHttpClient http, ILogger<Melody98Provider>? logger = null)
    {
        // insecureTls: CDN pages may serve self-signed certs (BUG-13 family).
        _http = http.Create("Melody98", insecureTls: true);
        SharedHttpClient.ApplyBrowserHeaders(_http, "https://melody98.ir/");
        _logger = logger ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<Melody98Provider>.Instance;
    }

    public bool CanDownload(SearchResult result) => result.Provider == ProviderId.Melody98;
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        if (html.Length == 0)
        {
            _logger.LogDebug("Melody98 empty response for {Query}", query);
            yield break;
        }

        var doc = new HtmlDocument();
        doc.LoadHtml(html);
        // melody98.ir result entries: <a href="https://melody98.ir/music/{id}/">
        var postNodes = doc.DocumentNode.SelectNodes("//a[contains(@href,'/music/')]");
        ...
        foreach (var node in postNodes)
        {
            ...
            if (!uri.Host.Contains("melody98.ir")) continue;
    ...
    {
        progress?.Report(new DownloadProgress(DownloadPhase.Resolving, 0, null, "Melody98: resolving URL"));
        ...
        {
            _logger.LogDebug("Melody98 re-resolve failed ({Msg}); falling back to stored URL", ex.Message);
        }
        finalUrl ??= track.DirectStreamUri?.OriginalString;
        if (string.IsNullOrEmpty(finalUrl))
            throw new InvalidOperationException("No download URL on Melody98 page.");
        ...
        return new DownloadResult(finalPath, StreamQuality.Maximum256K, ProviderId.Melody98);
    }
    ...
    private async Task<string> GetStringAsync(string url, CancellationToken ct)
    {
        ...
        {
            _logger.LogDebug("Melody98 HTTP failure for {Url}: {Msg}", url, ex.Message);
            return "";
        }
    }
    ...
    private async Task<TrackLink?> GetTrackAsync(string trackPageUrl, string slugText, CancellationToken ct)
    {
        ...
        // Look for download links in the page - Melody98 uses JavaScript to construct URLs
        var links = doc.DocumentNode.SelectNodes("//a[contains(@href,'dl.melody98.ir')]");
        if (links is null)
        {
        ...
        foreach (var link in links)
        {
            ...
            if (href.Contains("dl.melody98.ir") && href.EndsWith(".mp3", StringComparison.OrdinalIgnoreCase))
            {
                bestUrl = href;
    ...
    [GeneratedRegex("href=\"(https?://dl\\.melody98\\.ir[^\"]+\\.mp3)\"")]
